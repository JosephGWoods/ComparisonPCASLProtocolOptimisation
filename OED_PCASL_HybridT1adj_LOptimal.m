function [bestPLD,bestTau,bestminVariance] = OED_PCASL_HybridT1adj_LOptimal(varargin)
...
TTStep = 5; % We are only going to calculate the FIM for this many tau at a time, to save memory
...
for numPLD = nPLD
    ...
    tauAll = zeros(numPLD, 1, tauTryL);
    for jj = 1:tauTryL
        tauAll(:,1,jj) = bolus_fit(param, numPLD, tauTry(jj), 0, lims.tauStep);
    end
    tauAll(tauAll < 0.1) = 0.1; % Make sure no blocks are lower than 0.1
    tauAll = repmat( tauAll , 1 , PLDTryL , 1 );
    
    % For time-encoded data, there are effective PLDs
    PLD_effectiveShift = cumsum(tauAll(end:-1:1,:,:)) - tauAll(end:-1:1,:,:); % Count up backwards
    ...
    while continueFlag
        ...
        for ii = 1:param.multiPLD
            ...
            for TT = 1:TTStep:tauTryL
                disp(['PLD loop = ' ns(countLoop) ' | PLD opt = ' ns(ii) '/' ns(nPLD) ' | LD opt = ' ns(TT) '/' ns(tauTryL)])
                
                tauTryInd = TT:min( (TT+TTStep-1),  tauTryL); % Make sure not to exceed dimensions
                tauTryIndL = length(tauTryInd);
                
                distWeightCurr = repmat( permute(distWeight, [3,2,1]) , PLDTryL , min(TTStep,tauTryIndL));
                param.tau = repmat( tauAll(:,:,tauTryInd) , param.multiPLD , 1 , 1);
                
                % Update the effective PLDs with the PLDs
                PLDTry_curr = zeros(param.multiPLD*numPLD,PLDTryL,min(TTStep,tauTryIndL));
                for nn = 1:param.multiPLD
                    PLDTry_curr(((nn-1)*numPLD)+1:nn*numPLD,:,:) = PLD_effectiveShift(:,:,tauTryInd) + PLD(nn);
                end
                ...
                for kk = 1:allSliceL
                    ...
                    PLDTry_curr(ind(ii):ind(ii)+numPLD-1,:,:) = PLD_effectiveShift(:,:,tauTryInd) + repmat(PLDTry(:)', numPLD, 1, min(TTStep,tauTryIndL));
    ...
    if (bestminVariance-minVariance)/bestminVariance > 1e-12
        ...
        bestTau = tauAll(1:param.num_enc,row,col);
        bestPLD = PLD;
