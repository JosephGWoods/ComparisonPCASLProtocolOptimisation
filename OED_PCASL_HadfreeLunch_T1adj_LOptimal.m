function [bestPLD,bestTau,bestminVariance] = OED_PCASL_HadfreeLlnch_T1adj_LOptimal(varargin)
...
for numPLD = nPLD
    ...
    for jj = 1:tauTryL
        param.tau(2:end,1,jj) = bolus_fit(param.T1b, numPLD-1, tauTry(jj), 0, lims.tauStep); % 0 PLD
    end
    param.tau(param.tau < lims.tauLB) = lims.tauLB; % Make sure no blocks are lower than 0.1
    param.tau = repmat( param.tau , 1 , PLDTryL , 1 ); % Dimensions nPLD x PLDTryL x tauTryL
